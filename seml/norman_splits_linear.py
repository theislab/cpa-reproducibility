...
def run(
    ...
    split_key_int = int(split_key_int)
    adata = sc.read('/storage/groups/ml01/workspace/carlo.dedonno/cpa-reproducibility/datasets/Norman2019_prep_new.h5ad')
    save_dir = '/storage/groups/ml01/workspace/carlo.dedonno/cpa-reproducibility/seml/norman_splits_models/'
    cpa_api = cpa.api.API(
        adata,
        perturbation_key='condition',
        covariate_keys=['cell_type'],
        dose_key='dose_val',
        split_key=f'split{split_key_int}',
        loss_ae='gauss',
        doser_type='linear',
    ...
    #train model
    cpa_api.train(
        filename=f"model_linear_split{split_key_int}.pt",
        run_eval=True,
        max_epochs=2000,
        max_minutes=2*120,
    )
